...
static int handle_regular_locked(InodeSet *seen, pthread_mutex_t *seen_mu, const struct stat *st,
                                 uint64_t *acc, pthread_mutex_t *acc_mu) {
...
static int du_sync_sum_regular_bytes_sequential(const char *root_path, const DuOptions *opt,
    ...
    *out_bytes = 0;
    ...
    if (S_ISREG(sb.st_mode)) {
        int rc = handle_regular_locked(seen, &seen_mu, &sb, out_bytes, &acc_mu);
        inode_set_destroy(seen);
        stack_destroy(&st);
        return (rc == 0) ? 0 : 1;
    }
    ...
    for (;;) {
        ...
        for (struct dirent *de = readdir(dir); de; de = readdir(dir)) {
            ...
            if (S_ISREG(csb.st_mode)) {
                int rc = handle_regular_locked(seen, &seen_mu, &csb, out_bytes, &acc_mu);
                free(child);
                if (rc != 0) {
                    closedir(dir);
                    free(dirpath);
...
/* ---------- parallel traversal: work queue + threads ---------- */
...
typedef struct WorkQueue {
    DirNode *head;
    DirNode *tail;
    size_t pending;      /* number of queued dirs */
    size_t active;       /* number of workers currently processing a dir */
    int done;            /* all work complete */
    pthread_mutex_t mu;
    pthread_cond_t cv;
} WorkQueue;
...
static void *worker_main(void *arg) {
    ...
    for (;;) {
        ...
        if (fatal) {
            pthread_mutex_lock(&s->q->mu);
            s->q->done = 1;
            pthread_cond_broadcast(&s->q->cv);
            pthread_mutex_unlock(&s->q->mu);
            break;
        }
    }
    return NULL;
...
static int du_sync_sum_regular_bytes_parallel(const char *root_path, const DuOptions *opt,
    ...
    *out_bytes = 0;
    ...
    for (int i = 0; i < n; i++) {
        if (pthread_create(&threads[i], NULL, worker_main, &s) != 0) {
            pthread_mutex_lock(&q.mu);
            q.done = 1;
            pthread_cond_broadcast(&q.cv);
            pthread_mutex_unlock(&q.mu);
...
int du_sync_sum_regular_bytes(const char *root_path, const DuOptions *opt, uint64_t *out_bytes) {
    ...
    if (!threads) {
        ...
        pthread_mutex_destroy(&s.seen_mu);
        pthread_mutex_destroy(&s.total_mu);
        inode_set_destroy(seen);#define _XOPEN_SOURCE 700
...
int du_sync_sum_regular_bytes(const char *root_path, const DuOptions *opt, uint64_t *out_bytes) {
    if (!root_path || !out_bytes) return 2;#define _XOPEN_SOURCE 700
...
int du_sync_sum_regular_bytes(const char *root_path, const DuOptions *opt, uint64_t *out_bytes) {
    ...
    if (jobs <= 1) return du_sync_sum_regular_bytes_sequential(root_path, opt, out_bytes);
    return du_sync_sum_regular_bytes_parallel(root_path, opt, out_bytes);#define _XOPEN_SOURCE 700
